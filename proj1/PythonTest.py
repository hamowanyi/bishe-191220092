...
class jsTest():
    def __init__(self, filepath='D:/PyProjects/proj1/dataset/javascript/test.jsonl'):
        '''with open(filepath, "r+", encoding="utf8") as f:
            for item in jsonlines.Reader(f):
                print(item)
            f.close()
    ...
    def run(self, set=0, end=-1):
        ...
        thousand_count = 0
        print(len(df))
        pb_model = Prompt_base(language='java', tokenizer_str='Salesforce/codet5-large',
    ...
    def savetoFile(self, i):
        filename = 'jsresult1_' + str(i) + '.jsonl'
        df = pd.DataFrame(self.outputList)
        df.to_json(filename, double_precision=4, orient='records', lines=True)
